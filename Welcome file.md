...
## Files
The **csv2parquet** folder contains a script to generate an enriched Parquet with labels, as the dataset contains many fields that represent "codes" such as the district codes. In the dataset there is another excel sheet that explains what those codes are, which is decoded using the **map-config.json** to create label columns in the resulting Parquet. Typing of columns is done using **schema.json**.
